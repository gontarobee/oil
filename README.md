...
2. ページ内の **「石油備蓄の状況（推計値の速報）はこちら」** のリンクを開く（**最新の PDF**。ファイル名は公表日ごとに変わる。例: `pdf-oil-res/0323.pdf`）。
...
| ファイル | 内容 |
|----------|------|
| `script.js` | `RESERVE_DAYS`（合計日数）, `RESERVE_CAPACITY`（ゲージ100%の基準。通常は合計と同じ）, `REFERENCE`（起算日。日本時間0時を `Date` で指定）。シナリオ説明文に「241日分」等が直書きされていれば合わせて修正。 |
